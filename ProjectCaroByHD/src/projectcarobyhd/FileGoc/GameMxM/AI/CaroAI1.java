...
public class CaroAI1 {
    Random rand;
    private int nextX;
    private int nextY;
    private int maxdepth;
    private State root; // Trạng thái hiện tại của trò chơi
    private Heuristic1 heuristic;
    private HeuristicNoob1 heuristic1;

    
    public CaroAI1(int maxdepth) {
        rand = new Random();
        this.maxdepth = maxdepth;
	root = new State(); 
	// AI đánh mặc định nút ở giữa
	root.update(Value.SIZE/2, Value.SIZE/2, Value.AI2_VALUE);
	nextX = Value.SIZE/2;
	nextY = Value.SIZE/2;
        heuristic = new Heuristic1();
        heuristic1 = new HeuristicNoob1();
    }
    ...
    public void nextStep() {
    	...
    	if(choice == null) System.out.println("~ Lỗi! Không tìm thấy nước đi!");
    		...
    		if(!isClickable(this.nextX, this.nextY)) {
        		...
        		update(nextX, nextY, Value.AI1_VALUE); // cập nhật root
        	}
    	}
    }
    ...
    public Cell alphaBeta(State state) {
        if(this.maxdepth == 1){
            ...
            heuristic1.evaluateEachCell(remState, Value.AI1_VALUE);
            ...
            */
            ...
            for (int i = 0; i < n; i++) { // giả sử chọn một trong các phần tử trong list để tìm kiếm bước đi tối ưu
                // Giả sử AI đi ô này 
                    remState.getState()[list.get(i).getX()][list.get(i).getY()] = Value.AI1_VALUE;
            ...
            State remState = new State(state.getState()); // copy lại giá trị của state
            heuristic.evaluateEachCell(remState, Value.AI1_VALUE);
            //System.out.println("Bảng lượng giá các ô:");
            //heuristic.printEvalState(); // hiển thị bảng lượng giá
    ...
    private int maxValue(State state, int alpha, int beta, int depth) {
        if(this.maxdepth == 1){
            ...
            for (int i = 0; i < list.size(); i++) {
                state.getState()[list.get(i).getCell().getX()][list.get(i).getCell().getY()] = Value.AI1_VALUE;
    ...
    private int minValue(State state, int alpha, int beta, int depth) {
        if(this.maxdepth == 1){
            ...
            for (int i = 0; i < list.size(); i++) {
                state.getState()[list.get(i).getCell().getX()][list.get(i).getCell().getY()] = Value.AI2_VALUE;//  1
